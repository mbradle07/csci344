...
export default function LikeButton({post, token, requeryPost}) {
    ...
    return (
        <button role="switch" aria-checked={likeId ? 'true' : 'false'} aria-label={likeId ? 'liked' : 'unliked'} onClick={likeUnlike}><i className={likeId ? 'fas fa-heart' : 'far fa-heart'}></i></button>
    );
}