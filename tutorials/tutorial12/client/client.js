...
const handleServerMessage = ev => {
    ...
    if (data.type === "login") {
        ...
        const lis = data.active_users.map(item => `<li>${item}</li>`)
        document.querySelector('#users-list ul').innerHTML = lis.join('');
        console.log(data);
    } else if (data.type === "disconnect") {
        console.log('A user has just disconnected:');
        const lis = data.active_users.map(item => `<li>${item}</li>`)
        document.querySelector('#users-list').innerHTML = lis.join('');
        console.log(data);
